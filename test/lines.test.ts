...
describe("Lines Service Tests", () => {
	...
	it("should throw UnauthorizedError when a driver tries to create a line", () => {
		...
		expect(createLine(driverActor, lineToCreate)).rejects.toThrow(
			UnauthorizedError,
		);
	});
	it("should throw ResourceNotFoundError when getting a nonexistent line", () => {
		const adminActor = { ...seedData.admin, role: seedData.adminRole };
		const nonexistentLineId = v4();
		expect(getLine(adminActor, nonexistentLineId)).rejects.toThrow(
			ResourceNotFoundError,
		);
	});
});
