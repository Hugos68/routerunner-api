...
import { session, type Session } from "../database/tables/session.js";
import { pick, type Output } from "valibot";
...
export async function createSession(data: Output<typeof CredentialsSchema>) {
    const user_ = (await database.select().from(user).where(eq(user.email, data.email))).at(0);

    if (!user_) {
        throw new Error("User not found");
    }

    const matches = await Bun.password.verify(data.password, user_.password, 'argon2id');

    if (!matches) {
        throw new Error("Invalid credentials");
    }

    return await database.insert(session).values({
        userId: user_.id
    }).returning();
}
