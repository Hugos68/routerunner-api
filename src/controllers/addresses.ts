...
addresses.post("/", authorization("PLANNER", "ADMIN"), async (c) => {
	...
	return c.json(RouterunnerResponse.data(address), 201);
});

addresses.get("/", authorization("DRIVER", "PLANNER", "ADMIN"), async (c) => {
	const addresses = await getAddresses(c.req.query());
	return c.json(RouterunnerResponse.data(addresses), 200);
});
...
addresses.get(
	...
	async (c) => {
		...
		return c.json(RouterunnerResponse.data(address), 200);
	},
);
