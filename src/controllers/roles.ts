...
roles.post("/", authorization("ADMIN"), async (c) => {
	...
	return c.json(RouterunnerResponse.data(role), 201);
});

roles.get("/", authorization("ADMIN"), async (c) => {
	const roles = await getRoles(c.req.query());
	return c.json(RouterunnerResponse.data(roles), 200);
});

roles.get("/:id", authorization("ADMIN"), async (c) => {
	const id = c.req.param("id");
	const role = await getRole(id);
	return c.json(RouterunnerResponse.data(role), 200);
});
